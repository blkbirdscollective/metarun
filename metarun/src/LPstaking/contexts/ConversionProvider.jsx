...
export default function ConversionProvider({ children }) {
  ...
  const tokenToUSD = useCallback((n) => n * parseFloat(table.yarloo_price_usd || 0), [table])

  const tokenToBNB = useCallback((n) => n * parseFloat(table.yarloo_price_bnb || 0), [table])
...
export function useConversionContext() {
  ...
  if (context === undefined) {
    throw new Error('useConversionContext should be used inside an ConversionProvider')
  }
