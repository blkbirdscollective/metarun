...
export default function PoolContainer({ pool }) {
  ...
  const getTokenBalance = useCallback(async () => {
    ...
    try {
      ...
      setState((prev) => ({
        ...prev,
        tokenBalance: parseFloat(result / (1e18).toString()),
      }))
    } catch (error) {
      // console.log({ error });
    }
  ...
  const { tokenBalance, poolBalance, poolDepositTime, rewardEarned, totalDeposits, stakeLimit } = state
  ...
  const ttem = poolDepositTime
    ? format(pool.earlyMaturity.durationInMilliseconds + poolDepositTime, 'dd/MM/yyyy, HH:mm:ss')
    : '--'
  const ttm = poolDepositTime
    ? format(pool.maturity.durationInMilliseconds + poolDepositTime, 'dd/MM/yyyy, HH:mm:ss')
    : '--'

  const stakeDuration = addMillisecondsAndFormat(pool.maturity.durationInMilliseconds)
  ...
  return (
    <Pool
      ...
      stakeMinimal={pool.stakeMinimal}
